...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        ##Symbols33##
        #############
        print("Puzzle finished. Your score was: " + str(self.__Score))
        same = input("Play same again?").lower()
        if same == "y":
            for Count in range(len(self.__Grid)):
                if self.__Grid[Count].GetSymbol() != "@":
                    C = Cell()
                    # self.__Grid.pop(Count)
                    # self.__Grid.insert(Count, C)
                    self.__Grid[Count] = C
                    self.__Score = 0
                    self.__SymbolsLeft = 5
            self.AttemptPuzzle()
