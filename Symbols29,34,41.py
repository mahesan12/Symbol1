...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        #############
            changed = False
            if random.randrange(1, 21) < 5: #1 in 5 chance?
                valid = False
                while not valid:
                    randomcell = random.randrange(0, (self.__GridSize*self.__GridSize))
                    cell = self.__Grid[randomcell]
                    if cell.GetSymbol() == "-":
                        valid = True
                    else:
                        pass
                C = BlockedCell()
                self.__Grid.pop(randomcell)
                self.__Grid.insert(randomcell-1, C)
                changed = True

            self.DisplayPuzzle()
            if changed == True:
                row = math.ceil(((self.__GridSize*self.__GridSize)-randomcell)/8)
                col = randomcell%8
                print(f"Cell in (Row:{row},Col:{col}) has been blocked!")
        #############
        ##Symbols29##
        #############
