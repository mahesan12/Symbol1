...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ##Symbols24##
            ############
            if self.__SymbolsLeft == self.__TotalSymbols:
                Valid = False
                while not Valid:
                    try:
                        new = str(input(("Enter 'New' to create a new symbol, or Enter Key to skip: ")))
                        if (len(new) == 0) or new =='New':
                            Valid = True
                    except:
                        pass
                if new == "New":
                    Valid = False
                    while Valid == False:
                        print('Enter a 9 length string in a spiral pattern of the same letter and asteriks for blanks to make up the same larger letter, e.g: TTT**T**T')
                        pattern = str(input('Enter: ')).upper()
                        if len(pattern) == 9:
                            without_asteriks = pattern.strip("*")
                            if len(without_asteriks)>4:
                                Valid = True

                    letter = str(input("Enter the letter the pattern makes: ")).upper()
                    NewPattern = Pattern(letter, pattern)
                    self.__AllowedPatterns.append(NewPattern)
                    self.__AllowedSymbols.append(letter)
            ############
            ##Symbols24##
            ############
            Row = -1
