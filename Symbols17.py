    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if self.__Turns % 3 == 0:
                valid =False
                while not valid:
                    row = int(input("Enter row to get rid of @: "))
                    column = int(input("Enter column to get rid of @: "))
                    #for each_cell in self.__Grid:
                    current_cell = self.__GetCell(row,column)
                    if current_cell.GetSymbol() == "@":
                        valid = True
                    else:
                        print("Invalid cell")
                ...
                for each_cell in range(len(self.__Grid)):
                    if self.__Grid[each_cell] == current_cell:
                        self.__Grid[each_cell] = C
                change= True


            self.DisplayPuzzle()
            if change:
                print(f"@ removed at {row},{column}")
            #############
            ##Symbols17##
            #############
