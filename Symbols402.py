...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        Finished = False
        while not Finished:
            for each_cell in range(len(self.__Grid)):
                row = math.ceil((((self.__GridSize*self.__GridSize)-each_cell))/8)
                col = each_cell%8
                symbol = self.__Grid[each_cell].GetSymbol()
                if symbol !="-" and symbol !="@":
                    results = self.CheckforMatchWithPattern2(row, col)

                    pattern_list = results[0]
                    cells = results[1]
                    ...
                    for each_pattern in pattern_list:
                        ...
                        if symbol == "X":
                            for eachPattern in self.__AllowedPatterns:
                                if eachPattern.GetPatternSequence()[0] == "X":
                                    origPattern = eachPattern
                            if origPattern.GetPatternSequence()[pos] == "X":
                                for i in range(0,len(each_pattern),2):
                                    if each_pattern[i] == "-" or each_pattern[i] == "X":
                                        count +=1
                                if count ==5:
                                    Xs = 0
                                    for i in range(0,len(each_pattern), 2):
                                        if each_pattern[i] == "X":
                                            Xs +=1
                                if Xs >= 4:
                                    randomindex = random.randrange(0, 9)


                        elif symbol == "Q":
                            for eachPattern in self.__AllowedPatterns:
                                if eachPattern.GetPatternSequence()[0] == "Q":
                                    origPattern = eachPattern
                            if origPattern.GetPatternSequence()[pos] == "Q":
                                if (each_pattern[0] == "-" or each_pattern[0] == "Q") and (each_pattern[1] == "-" or each_pattern[1] == "Q")and (each_pattern[4] == "-" or each_pattern[4] == "Q")and (each_pattern[7] == "-" or each_pattern[7] == "Q")and (each_pattern[8] == "-" or each_pattern[8] == "Q"):
                                    possible = True
                                    nopatterns +=1
                        
                        elif symbol == "T":
                            for eachPattern in self.__AllowedPatterns:
                                if eachPattern.GetPatternSequence()[0] == "T":
                                    origPattern = eachPattern
                            if origPattern.GetPatternSequence()[pos] == "T":
                                if (each_pattern[0] == "-" or each_pattern[0] == "T") and (each_pattern[1] == "-" or each_pattern[1] == "T")and (each_pattern[2] == "-" or each_pattern[2] == "T")and (each_pattern[5] == "-" or each_pattern[5] == "T")and (each_pattern[8] == "-" or each_pattern[8] == "T"):
                                    possible = True
                                    nopatterns+=1
        ...
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                try:
                    ...
                    list2.append(self.__GetCell(StartRow, StartColumn))
                    list2.append(self.__GetCell(StartRow, StartColumn+1))
                    list2.append(self.__GetCell(StartRow, StartColumn+2))
                    list2.append(self.__GetCell(StartRow-1, StartColumn+2))
                    list2.append(self.__GetCell(StartRow-2, StartColumn+2))
                    list2.append(self.__GetCell(StartRow-2, StartColumn+1))
                    list2.append(self.__GetCell(StartRow-2, StartColumn))
                    list2.append(self.__GetCell(StartRow-1, StartColumn))
                    list2.append(self.__GetCell(StartRow-1, StartColumn+1))
                    
                    list.append(PatternString)
                except:
                    pass
        return list, list2
