...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        ##Symbols16##
        #############
        with open("puzzleLeaderboard.txt") as f:
            view_scores = {}
            count = int(f.readline().rstrip())
            for Count in range (1, count + 1):
                score = (f.readline().rstrip().split(":"))
                view_scores[score[0]] = int(score[1])
        sorted_dict = dict(sorted(view_scores.items(), key=lambda item: item[1], reverse=True))
    ...
    ##Symbols16##
    #############
    def AddToScoreBoard(self):
        filepath = 'puzzleLeaderboard.txt'
        Scores = {}
        ...
        for key in sorted_dict.keys():
            number = key.split()
            games.append(int(number[1]))
        games.sort(reverse=True)
        new_game = games[0] + 1
        sorted_dict[f"Game {new_game}"]= self.__Score
        print(sorted_dict)
        sorted_dict2 = dict(sorted(sorted_dict.items(), key=lambda item: item[1], reverse=True))
        print(sorted_dict2)
        with open(filepath, "w") as file:
            count +=1
            file.write(f"{count}\n")
            for key in sorted_dict2.keys():
                file.write(f"{key}:{sorted_dict2[key]}\n")
    #############
    ##Symbols16##
    #############
