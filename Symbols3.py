...
class Puzzle():
    ...
    def Bomb(self, row, column):
        ...
        CurrentCell.ChangeSymbolInCell("o")

        CurrentCell = self.__GetCell(row-1, column)
        CurrentCell.ChangeSymbolInCell("o")

        CurrentCell = self.__GetCell(row-1, column-1)
        CurrentCell.ChangeSymbolInCell("o")

        CurrentCell = self.__GetCell(row, column-1)
        CurrentCell.ChangeSymbolInCell("o")
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            ##Symbols3##
            ############
            bomb = input("Would you like to use a bomb? y for yes or enter to skip: ")
            if bomb == "y":
                if self.__Score>=10:
                    Valid = False
                    while not Valid:
